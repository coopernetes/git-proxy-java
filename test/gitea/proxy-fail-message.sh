#!/usr/bin/env bash
# Commit message validation failures via Gitea transparent proxy.
set -euo pipefail
...
print_header "TRANSPARENT PROXY: COMMIT MESSAGE FAILURES" "${PUSH_URL}"

run_test() {
    local test_name="$1"; shift
    setup_repo "${PUSH_URL}" "proxy-msg"
    "$@"
